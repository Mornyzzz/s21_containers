...
TEST(map, MapCapacity) {
  ...
  EXPECT_EQ(my_map.size(), (const unsigned long)1);
}
...
TEST(map, SwapMap) {
  ...
  EXPECT_EQ(my_map.size(), (const unsigned long)2);
  EXPECT_EQ(my_swap_map.size(), (const unsigned long)1);
...
TEST(map, InsertMany) {
  ...
  auto results =
      my_map.insert_many(std::make_pair(1, 'x'), std::make_pair(2, 'b'),
